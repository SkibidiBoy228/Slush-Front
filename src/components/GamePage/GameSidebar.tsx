...
function GameSideBar({game}:GameSidebarProps){
    ...
    const formattedOldPrice = `${game.oldPrice.toFixed(2)}₴`;
    ...
    return(
        <aside className="game-sidebar">
            <div className="game-rating">
                ...
                <div className="rating-stars">
                    {Array.from({length: 5}).map((_,index)=>(
                        <span
                            key={index}
                            className={index<Math.round(game.averageRating) ? "filled" : ""}>
                                ★
                            </span>
                    ))}
                </div>
            </div>
            <img className="sidebar-cover"
                src={game.thumbnail}
                alt={game.title}
                />
            <div className="sidebar-price">
                {game.discountPercent > 0 && (
                    <span className="sidebar-discount">-{game.discountPercent}%</span>
                )}
            ...
            <div className="sidebar-actions">
                ...
                <button className="wishlist-button" aria-label="Додати в обране">
                    {game.isInWishlist ? "♥" : "♡"}
                </button>
            </div>
